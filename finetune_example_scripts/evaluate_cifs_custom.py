...
def eval_cif(progress_queue, task_queue, result_queue,
             length_lo, length_hi, angle_lo, angle_hi, debug):
    ...
    while not task_queue.empty():
        ...
        try:
            ...
            # If valid, store (index, cif) for later saving
            if valid:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_valid_dir", required=False, default=None,
                        help="If specified, saves all CIFs with 'is_valid' == True to this directory")
    ...
    debug = args.debug
    save_valid_dir = args.save_valid_dir
    ...
    processes = [
        mp.Process(
            target=eval_cif,
            args=(
                progress_queue, task_queue, result_queue,
                length_lo, length_hi, angle_lo, angle_hi, debug
            )
    ...
    print(f"space group consistent: {n_space_group_consistent}/{n} "
          f"({n_space_group_consistent / n:.3f})\n"
          f"atom site multiplicity consistent: "
          f"{n_atom_site_multiplicity_consistent}/{n} "
          f"({n_atom_site_multiplicity_consistent / n:.3f})\n"
          f"avg. bond length reasonableness score: "
          f"{np.mean(bond_length_reasonableness_scores):.4f} "
          f"± {np.std(bond_length_reasonableness_scores):.4f}\n"
          f"bond lengths reasonable: "
          f"{bond_length_reasonableness_scores.count(1.)}/{n} "
          f"({bond_length_reasonableness_scores.count(1.) / n:.3f})")
    print(f"num valid: {n_valid}/{n} ({n_valid / n:.2f})")
    print(f"longest valid generated length: "
          f"{np.max(valid_gen_lens) if len(valid_gen_lens) > 0 else np.nan:,}")
    print(f"avg. valid generated length: "
          f"{np.mean(valid_gen_lens):.3f} ± {np.std(valid_gen_lens):.3f}")

    # Save results to CSV
    pd.DataFrame(results_data).to_csv(out_fname)

    # Optionally save valid CIFs
    if save_valid_dir is not None:
        os.makedirs(save_valid_dir, exist_ok=True)

        # Set to track filenames already used
        used_filenames = set()

        for i, valid_cif in valid_cif_indices:
            # Extract the composition safely from the CIF content
            try:
                safe_comp = valid_cif.split("\n")[0].split("data_")[1].strip()
            except IndexError:
                safe_comp = f"unknown_{i}"  # Fallback in case data_ line is missing
            
            # Ensure the filename is unique
            filename = f"{safe_comp}.cif"
            counter = 1
            while filename in used_filenames:
                filename = f"{safe_comp}_{counter}.cif"
                counter += 1
            
            # Add the filename to the set of used filenames
            used_filenames.add(filename)
            
            # Write the valid CIF to the directory
            with open(os.path.join(save_valid_dir, filename), "w") as f:
                f.write(valid_cif)
